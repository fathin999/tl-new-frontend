...
import {ref} from "vue";
import {getCourseFees} from "@/composable/backend/courses";
import {getCountries} from "@/composable/view-models/course";

// PROPS
const props = defineProps({
    type: {type: String, default: "Certified Associate"},
});

// ------------------------------
// BACKEND
// ------------------------------
// BACKEND - GET COURSE FEES
const fees = getCourseFees(props.type);
...
const getCountryImg = (img) => {
    return new URL(
        `/src/assets/database/courses/flags/flag-${img}.png`,
        import.meta.url
    );
};
...
<template>
    ...
    <div
        id="course-fees-list"
        v-if="type === 'M1' || type === 'Certified Associate'"
    >
        <div
            v-for="fee in fees"
            :key="fee.title"
    ...
    <div
        id="course-fees-para"
        v-html="fees"
        v-if="type === 'Foundation Certificate'"
    ></div>
</template>
